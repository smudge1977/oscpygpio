...
'''
Title:         oscpygpio.py
Description:   OSC Implementation of GPIO control on the PI 
			   designed to work with companion as the OSC source
...
Be nice to: dispatch gpio_read(pin) seperatly 
	so after inital init compaion will follow actual pin states
...
def gpio_read(pin,*args):
    ...
    #* Would be nice to send back to the broadcast if sent to the broadcast
    #  and unicast if sent to the unicast address
    ...
    with open(args_dict['config']) as f:
        conf = json.load(f)
    client = udp_client.SimpleUDPClient("192.168.0.255",
			args_dict.get('replyport'), allow_broadcast=True) #* Need to use what it is told from config!
    ...
    if pin_state == 1:
        try:
            ...
            for action in actions:
                ...
                logging.info(str('sendresponse: to: {} value: {}'
						.format(action['path'], action['value'])))
        ...
        logging.critical(str('pin: "{}", pin_state: "{}", message: "UNKOWN STATE!! "'
				.format(pin, pin_state)))
...
def gpio_set(*args):
    ...
    if value in ('1','HIGH'):
        ...
        logging.info(str('pin: "{}", set: "HIGH", by: "{}", with: "{}"'
				.format(pin, server.server_address, args)))
    else:
        gpio.output(pin,gpio.LOW)
        logging.info(str('pin: "{}", set: "LOW", by: "{}", with: "{}"'
            	.format(pin, server.server_address, args)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--companion', default='192.168.0.81',
        help='Companion IP - this is for sending back the OSC to give feed back '
            '#* would be nice to know the source of the OSC and just send back to that rather than specify')
    parser.add_argument('--config', default='oscpygpio.conf',
        help='config from a file - #* too be implemented')
    parser.add_argument('--log', default='DEBUG',
        help='Set log level i.e. DEBUG or INFO - ' 
            	'INFO would seem sencible for normal systemctl running')
